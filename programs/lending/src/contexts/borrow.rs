...
impl<'info> Borrow<'info> {
    pub fn borrow(&mut self, amount: u64) -> Result<()> {
        ...
        match self.mint.to_account_info().key() {
            ...
            _ => {
                ...
                let usdc_price = self.price_update.get_price_no_older_than(&Clock::get()?, MAX_AGE, &usdc_feed_id)?;
                let new_value = self.calculate_accrued_interest(self.user.deposited_usdc, self.bank.interest_rate, self.user.last_updated)?;
                total_collateral = usdc_price.price as u64 * new_value;
            }
        }

        let borrowable_amount = total_collateral.checked_mul(self.bank.liquidation_threshold).unwrap();
        ...
        let user_shares = self.bank.total_borrowed_shares.checked_mul(borrowed_ratio).unwrap();
    ...
    fn calculate_accrued_interest(&mut self, deposited: u64, interest_rate: u64, last_updated: i64) -> Result<u64> {
